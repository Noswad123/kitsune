...
Kitsune started as a Herdr fork and continues the same goal: a fast, simple, effective workspace manager for coding agents. I care a lot about how it looks, feels, and works, so many design and technical decisions here are deliberate.

This guide exists so I can keep Kitsune manageable as a solo project and keep it from drifting from what it is supposed to be.

## The One Rule

**You must understand your code.** If you cannot explain what your changes do, how they behave at the edges, and how they fit Kitsune's existing design, your PR will be closed.

Using AI to write code is fine. Submitting code you do not understand is not.

## Kitsune is opinionated

Kitsune has a specific direction for how it should look, feel, and work.
...
Kitsune does not carry Herdr's public website or versioned website docs. Keep
durable project notes in root docs or `.local/prd/`, and keep the bundled API
schema under `docs/next/api/` when the socket API changes.
...
Do not use GitHub closing keywords like `fixes #128`, `closes #128`, or `resolves #128` in normal PR commits. Kitsune closes released issues after a release is published, not when unreleased commits land on `master`.
...
If a PR introduces a feature without prior alignment, or changes Kitsune's feel without discussion, it will likely be closed.
