...
const SOURCE_CHECKOUT = "/home/can/Projects/kitsune";
const DEFAULT_BASE = "master";
const EXTENSION_DIR = dirname(fileURLToPath(import.meta.url));
const EXTENSION_NAME = "kitsune-worktree";

type KitsuneWorktreeResult = {
  result?: {
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    name: "kitsune_start_worktree",
    label: "Start Kitsune Worktree",
    description:
      "Create a Kitsune-linked git worktree from the Kitsune master checkout, continue the active pi session in it, " +
      "start pi in the new Kitsune pane, then shut down and clean up the old pane.",
    promptSnippet: "Create a Kitsune worktree workspace and continue the active pi session in it",
    promptGuidelines: [
      "Use kitsune_start_worktree when work in the Kitsune repo should continue in a fresh git worktree.",
      "kitsune_start_worktree creates the checkout from /home/can/Projects/kitsune on master by default.",
      "Prefer passing a clear branch name such as issue/123-short-slug when the work relates to an issue.",
      "After kitsune_start_worktree succeeds, the current pi process will shut down and the old Kitsune pane will close.",
    ],
    parameters: Type.Object({
      branch: Type.Optional(
        Type.String({
          description:
            "Branch name for the new worktree. If omitted, Kitsune generates a worktree/* branch.",
        }),
      ),
      base: Type.Optional(
        Type.String({
      ...
      label: Type.Optional(
        Type.String({
          description: "Workspace label for the new Kitsune worktree workspace.",
        }),
      ),
      closeOldPane: Type.Optional(
        Type.Boolean({
          description: "Close the old Kitsune pane after the old pi process exits. Defaults to true.",
        }),
      ),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      return startKitsuneWorktree(pi, ctx, signal, {
  ...
  pi.registerCommand("kitsune-worktree-start", {
    description:
      "Create a Kitsune worktree from master, continue this pi session in it, and clean up the old pane",
    handler: async (args, ctx) => {
      await ctx.waitForIdle();
      try {
        const parsed = parseCommandArgs(args ?? "");
        const result = await startKitsuneWorktree(pi, ctx, undefined, parsed);
...
async function startKitsuneWorktree(
  pi: ExtensionAPI,
  ...
  ctx.ui.setStatus("kitsune-worktree", "creating worktree");

  let newSessionFile: string | undefined;
  try {
    const created = await createKitsuneWorktree(pi, signal, sourceCheckout, options);
    ...
    ctx.ui.setStatus("kitsune-worktree", undefined);
    ctx.ui.notify(`Started pi in Kitsune worktree: ${worktreePath}`, "info");
    ctx.shutdown();
      ...
      content: [
        {
          type: "text" as const,
          text:
            `Started replacement pi in Kitsune worktree: ${worktreePath}\n` +
            `Workspace: ${created.workspaceId ?? "unknown"}\n` +
            `Pane: ${created.rootPaneId}\n` +
            `Branch: ${created.branch ?? "generated by Kitsune"}\n\n` +
...
async function createKitsuneWorktree(
  pi: ExtensionAPI,
  signal: AbortSignal | undefined,
  sourceCheckout: string,
  options: StartOptions,
  ...
  const json = await kitsuneJson(pi, args, signal, sourceCheckout, 120_000);
  const worktreePath = json.result?.worktree?.path;
  const rootPaneId = json.result?.root_pane?.pane_id;
  if (!worktreePath || !rootPaneId) {
    throw new Error("Kitsune worktree create response did not include worktree.path and root_pane.pane_id");
  }
...
async function runInNewPane(
  ...
  await kitsune(pi, ["pane", "run", paneId, command], signal, undefined, 10_000);
}
...
async function scheduleOldPaneCleanup(
  ...
  const cleanup = [
    ...
    "rm -f -- \"$old_session\"",
    "kitsune pane close \"$old_pane\" >/dev/null 2>&1 || true",
  ].join("; ");
...
async function kitsuneJson(
  pi: ExtensionAPI,
  args: string[],
  signal: AbortSignal | undefined,
  cwd: string | undefined,
  timeout: number,
): Promise<KitsuneWorktreeResult> {
  const result = await kitsune(pi, args, signal, cwd, timeout);
  const raw = result.stdout.trim() || result.stderr.trim();
  let json: KitsuneWorktreeResult;
  try {
    json = JSON.parse(raw) as KitsuneWorktreeResult;
  } catch {
    throw new Error(`Kitsune returned non-JSON output for ${args.join(" ")}: ${raw}`);
  }
  if (json.error) {
    throw new Error(`${json.error.code ?? "kitsune_error"}: ${json.error.message ?? "unknown Kitsune error"}`);
  }
  return json;
}
...
async function kitsune(
  ...
  const result = await pi.exec("kitsune", args, { cwd, signal, timeout });
  if (result.code !== 0) {
    throw new Error(`kitsune ${args.join(" ")} failed: ${result.stderr || result.stdout}`);
  }
  return result;
}
