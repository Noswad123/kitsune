...
ポップアップコマンドは `KITSUNE_PANE_ID` を受け取りません。背後のタイルペインには `KITSUNE_ACTIVE_PANE_ID` を使ってください。
...
カスタムコマンドは、利用可能な場合に `KITSUNE_SOCKET_PATH`、`KITSUNE_BIN_PATH`、`KITSUNE_ACTIVE_WORKSPACE_ID`、`KITSUNE_ACTIVE_TAB_ID`、`KITSUNE_ACTIVE_PANE_ID`、`KITSUNE_ACTIVE_PANE_CWD` を受け取ります。Herdr が検出できる場合、シェルコマンドはフォーカス中のペインの作業ディレクトリから実行されます。

Windows では、カスタムコマンド文字列に `cmd.exe /d /c` が使われるため、環境変数には `%KITSUNE_BIN_PATH%` 構文を使います。PowerShell 構文を実行するには、たとえば `powershell.exe -NoProfile -Command "..."` のように明示的に呼び出してください。
...
| 変数 | 目的 |
| --- | --- |
| `KITSUNE_CONFIG_PATH` | 設定ファイルのパスを上書きする。 |
| `KITSUNE_SESSION` | CLI コマンドで名前付きセッションを選択する。 |
| `KITSUNE_SOCKET_PATH` | 低レベルなソケットパスを上書きする。 |
| `KITSUNE_LOG` | ログフィルタリングを設定する。例: `KITSUNE_LOG=herdr=debug`。 |
| `KITSUNE_DISABLE_SOUND` | `[ui.sound] enabled = true` でも音の再生を無効にする。 |
