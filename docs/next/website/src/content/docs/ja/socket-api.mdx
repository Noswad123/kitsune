...
プロセスを起動するメソッドは `env` オブジェクトを受け付けます。Herdr はそのキー/値ペアを新しく起動されるプロセスにのみ適用します。Herdr は管理下のペインプロセスに `KITSUNE_SOCKET_PATH`、`KITSUNE_ENV=1`、`KITSUNE_WORKSPACE_ID`、`KITSUNE_TAB_ID`、`KITSUNE_PANE_ID` も注入します。呼び出し側が与えた環境変数と衝突した場合、Herdr 管理の変数が権威を持ちます。
...
`source` は所有者を識別します。プラグインは `plugin:<KITSUNE_PLUGIN_ID>` を使います。
...
クエリを `KITSUNE_PLUGIN_STATE_DIR` に保存し、`[[startup]]` フックから再適用してください。
...
Herdr は `KITSUNE_SOCKET_PATH`、`KITSUNE_BIN_PATH`、`KITSUNE_ENV=1`、`KITSUNE_PLUGIN_ID`、`KITSUNE_PLUGIN_ROOT`、`KITSUNE_PLUGIN_CONFIG_DIR`、`KITSUNE_PLUGIN_STATE_DIR`、`KITSUNE_PLUGIN_CONTEXT_JSON`、そして利用可能な `KITSUNE_WORKSPACE_ID`、`KITSUNE_TAB_ID`、`KITSUNE_PANE_ID` の値を注入します。アクションコマンドは加えて `KITSUNE_PLUGIN_ACTION_ID` を受け取ります。イベントフックは `KITSUNE_PLUGIN_EVENT` と `KITSUNE_PLUGIN_EVENT_JSON` を、ペインコマンドは `KITSUNE_PLUGIN_ENTRYPOINT_ID` を受け取ります。
...
v1 には Herdr が管理するプラグインストレージ API はありません。`KITSUNE_PLUGIN_CONFIG_DIR` と `KITSUNE_PLUGIN_STATE_DIR` はパスの発見だけを提供します。ファイル、スキーマ、マイグレーション、クリーンアップはプラグインが所有します。
...
`KITSUNE_SOCKET_PATH` は低レベルな上書きにのみ使ってください。

プラグインでは、移植性のある Windows の挙動が必要なときは `KITSUNE_BIN_PATH` と CLI ラッパーの呼び出しを推奨します。生のソケットクライアントは、プラットフォームネイティブのローカルソケット形式を使う責任を負います。
