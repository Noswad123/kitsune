...
`KITSUNE_BIN_PATH` を通じて Herdr を呼び出すべきです。これにより、Unix ソケットと
...
コマンドの中からは `KITSUNE_BIN_PATH` で Herdr を呼び出します:
...
たとえば宣言型の Agent ビューを `KITSUNE_PLUGIN_STATE_DIR` に保存し、スタートアップフックで
読み込んで再適用できます。

スタートアップフックは通常のランタイムプラグイン環境と
`KITSUNE_PLUGIN_EVENT=startup` を受け取ります。インストールプレビューには、自動実行される
コードを確認できるよう、すべてのスタートアップコマンドが表示されます。

## コマンドと環境

ランタイムコマンドは、プラグインディレクトリを作業ディレクトリとして実行されます。
Herdr は `KITSUNE_SOCKET_PATH`、`KITSUNE_BIN_PATH`、`KITSUNE_ENV=1`、
`KITSUNE_PLUGIN_ID`、`KITSUNE_PLUGIN_ROOT`、`KITSUNE_PLUGIN_CONFIG_DIR`、
`KITSUNE_PLUGIN_STATE_DIR`、`KITSUNE_PLUGIN_CONTEXT_JSON`、そして利用可能なら
`KITSUNE_WORKSPACE_ID`、`KITSUNE_TAB_ID`、`KITSUNE_PANE_ID` を注入します。アクション
コマンドは加えて `KITSUNE_PLUGIN_ACTION_ID` を受け取ります。スタートアップフックと
イベントフックは `KITSUNE_PLUGIN_EVENT`（スタートアップフックでは `startup`）を受け取り、
イベントフックはさらに `KITSUNE_PLUGIN_EVENT_JSON` を、ペインコマンドは
`KITSUNE_PLUGIN_ENTRYPOINT_ID` を受け取ります。

`KITSUNE_PLUGIN_ROOT` は、インストールまたはリンクされたプラグインディレクトリです。
GitHub からインストールされたプラグインのルートは管理されたソースチェックアウト
なので、そこにユーザーの認証情報や永続状態を保存しないでください。`.env` ファイルの
ようなユーザーが編集する設定は `KITSUNE_PLUGIN_CONFIG_DIR` の下に、ローカルの
ランタイム状態は `KITSUNE_PLUGIN_STATE_DIR` の下に置いてください。Herdr はこれらの
ディレクトリを作成し、レガシーなプラグイン設定の場所が存在すれば
`KITSUNE_PLUGIN_CONFIG_DIR` に初期内容を移しますが、その中身の検証、同期、削除は
しません。ファイル形式とライフサイクルはプラグインが所有します。

`KITSUNE_PLUGIN_CONTEXT_JSON` には、その呼び出しで利用可能な場合、ワークスペース、
...
あるときは `KITSUNE_BIN_PATH` を使ってください。`KITSUNE_SOCKET_PATH` の背後にある生の
ソケットトランスポートは OS 固有です: Unix クライアントは Unix ソケットパスに、
Windows クライアントは名前付きパイプに接続します。`KITSUNE_BIN_PATH` を通じた CLI
...
そのプロセスには `KITSUNE_PANE_ID` が渡されず、背後のタイルペインは `KITSUNE_PLUGIN_CONTEXT_JSON` から参照できます。
...
ハンドラーのアクションは `KITSUNE_PLUGIN_CONTEXT_JSON` で
`invocation_source = "link_click"`、`clicked_url`、`link_handler_id` を受け取ります。
シェルプラグインは `KITSUNE_PLUGIN_CLICKED_URL` と `KITSUNE_PLUGIN_LINK_HANDLER_ID` も
