...
`KITSUNE_PANE_ID` when the command runs inside a Herdr pane. For `pane split`,
...
After `pane move`, use `.result.move_result.pane.pane_id` for later commands. A cross-workspace move changes the workspace-qualified pane ID; the prior value remains at `.result.move_result.previous_pane_id`. The running process keeps its launch-time `KITSUNE_PANE_ID`, `KITSUNE_TAB_ID`, and `KITSUNE_WORKSPACE_ID`; Herdr retains the old pane ID as an alias for that terminal, so pane commands using `--current` still resolve it. A live agent name follows the terminal and continues to resolve after the move.
...
is not a Herdr pane, does not export `KITSUNE_PANE_ID`, and does not participate
in pane or agent APIs. Native non-terminal plugin panes are a later surface.

`--env KEY=VALUE` can be repeated on process-launching commands. It applies to the newly launched process only. Herdr-managed variables such as `KITSUNE_SOCKET_PATH`, `KITSUNE_BIN_PATH`, `KITSUNE_ENV`, `KITSUNE_WORKSPACE_ID`, `KITSUNE_TAB_ID`, `KITSUNE_PANE_ID`, `KITSUNE_PLUGIN_ID`, `KITSUNE_PLUGIN_ROOT`, `KITSUNE_PLUGIN_CONFIG_DIR`, `KITSUNE_PLUGIN_STATE_DIR`, `KITSUNE_PLUGIN_ENTRYPOINT_ID`, and `KITSUNE_PLUGIN_CONTEXT_JSON` stay authoritative when they conflict with caller-provided env.
...
| Variable | Purpose |
| --- | --- |
| `KITSUNE_CONFIG_PATH` | Override the config file path. |
| `KITSUNE_SESSION` | Select a named session for CLI commands. |
| `KITSUNE_SOCKET_PATH` | Low-level socket path override. |
| `KITSUNE_ENV` | Set to `1` inside Herdr-managed pane processes. |
| `KITSUNE_PANE_ID` | Public pane id for the running pane process. |
| `KITSUNE_TAB_ID` | Public tab id for the running pane process. |
| `KITSUNE_WORKSPACE_ID` | Public workspace id for the running pane process. |
| `KITSUNE_LOG` | Set log filter, for example `KITSUNE_LOG=herdr=debug`. |
| `KITSUNE_DISABLE_SOUND` | Disable sound playback even when sound notifications are enabled. |
