...
kitsune api schema --output kitsune-api.schema.json
```
...
{"id":"req_split","method":"pane.split","params":{"direction":"right","ratio":0.333,"env":{"KITSUNE_ROLE":"tests"}}}
...
pairs to the newly launched process only. Herdr also injects `KITSUNE_SOCKET_PATH`,
`KITSUNE_ENV=1`, `KITSUNE_WORKSPACE_ID`, `KITSUNE_TAB_ID`, and `KITSUNE_PANE_ID` into
...
`title` is required and must contain visible text after control characters and repeated whitespace are removed. `body` is optional. Herdr collapses newlines, tabs, carriage returns, and repeated whitespace into spaces, then trims notification text to 80 characters for `title` and 240 characters for `body`. An empty sanitized `title` returns `invalid_params`. `position` is optional and applies only when `ui.toast.delivery = "kitsune"`; desktop positions are relative to the full Herdr frame, and omitted positions use `ui.toast.kitsune.position`. Terminal, system, and off delivery ignore `position`. `sound` is optional and can be `none`, `done`, or `request`; it defaults to `none` and plays only when the notification is shown.
...
`source` identifies the owner. Plugins use `plugin:<KITSUNE_PLUGIN_ID>`; Herdr
...
durable behavior should save the query under `KITSUNE_PLUGIN_STATE_DIR` and
...
Herdr injects `KITSUNE_SOCKET_PATH`, `KITSUNE_BIN_PATH`, `KITSUNE_ENV=1`,
`KITSUNE_PLUGIN_ID`, `KITSUNE_PLUGIN_ROOT`, `KITSUNE_PLUGIN_CONFIG_DIR`,
`KITSUNE_PLUGIN_STATE_DIR`, `KITSUNE_PLUGIN_CONTEXT_JSON`, and available
`KITSUNE_WORKSPACE_ID`, `KITSUNE_TAB_ID`, and `KITSUNE_PANE_ID` values. Action
commands also receive `KITSUNE_PLUGIN_ACTION_ID`; event hooks receive
`KITSUNE_PLUGIN_EVENT` and `KITSUNE_PLUGIN_EVENT_JSON`; pane commands receive
...
There is no Herdr-managed plugin storage API in v1. `KITSUNE_PLUGIN_CONFIG_DIR`
and `KITSUNE_PLUGIN_STATE_DIR` are path discovery only; plugins own their files,
schemas, migrations, and cleanup.

Open a managed terminal UI:

```json
{"id":"req_plugin_pane","method":"plugin.pane.open","params":{"plugin_id":"example.board","entrypoint":"board","placement":"zoomed","target_pane_id":"w1:p1","env":{"KITSUNE_ROLE":"board"},"focus":true}}
```
...
`KITSUNE_PANE_ID` to its process. Popup launch returns `ok`; `popup.close` closes
...
2. `KITSUNE_SOCKET_PATH`
3. `KITSUNE_SESSION=<name>`
4. default session socket

Use `KITSUNE_SOCKET_PATH` only for low-level overrides.

For plugins, prefer invoking `KITSUNE_BIN_PATH` and the CLI wrappers when you need
