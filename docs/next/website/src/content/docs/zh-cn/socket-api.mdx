...
启动进程的方法接受一个 `env` 对象。Herdr 只把这些键值对应用到新启动的进程。Herdr 还向受管窗格进程注入 `KITSUNE_SOCKET_PATH`、`KITSUNE_ENV=1`、`KITSUNE_WORKSPACE_ID`、`KITSUNE_TAB_ID` 和 `KITSUNE_PANE_ID`。与调用方提供的环境变量冲突时,Herdr 管理的变量保持权威。
...
`title` 必填,并且在移除控制字符和重复空白后必须仍有可见文本。`body` 可选。Herdr 把换行、制表符、回车和重复空白折叠为空格,然后把通知文本截断: `title` 80 个字符,`body` 240 个字符。净化后为空的 `title` 返回 `invalid_params`。`position` 可选,只在 `ui.toast.delivery = "kitsune"` 时生效;桌面位置相对于完整的 Herdr 画面,省略时使用 `ui.toast.kitsune.position`。terminal、system 和 off 投递忽略 `position`。`sound` 可选,取值 `none`、`done` 或 `request`;默认 `none`,且只在通知实际显示时播放。
...
`source` 标识所有者。插件使用 `plugin:<KITSUNE_PLUGIN_ID>`;当所属插件不存在
...
`KITSUNE_PLUGIN_STATE_DIR`,并从 `[[startup]]` 钩子重新应用。
...
Herdr 注入 `KITSUNE_SOCKET_PATH`、`KITSUNE_BIN_PATH`、`KITSUNE_ENV=1`、`KITSUNE_PLUGIN_ID`、`KITSUNE_PLUGIN_ROOT`、`KITSUNE_PLUGIN_CONFIG_DIR`、`KITSUNE_PLUGIN_STATE_DIR`、`KITSUNE_PLUGIN_CONTEXT_JSON`,以及可用的 `KITSUNE_WORKSPACE_ID`、`KITSUNE_TAB_ID` 和 `KITSUNE_PANE_ID` 值。动作命令还收到 `KITSUNE_PLUGIN_ACTION_ID`;事件钩子收到 `KITSUNE_PLUGIN_EVENT` 和 `KITSUNE_PLUGIN_EVENT_JSON`;窗格命令收到 `KITSUNE_PLUGIN_ENTRYPOINT_ID`。
...
v1 没有 Herdr 管理的插件存储 API。`KITSUNE_PLUGIN_CONFIG_DIR` 和 `KITSUNE_PLUGIN_STATE_DIR` 只提供路径发现;文件、schema、迁移和清理归插件所有。
...
`KITSUNE_SOCKET_PATH` 只用于底层覆盖。

对插件来说,需要可移植的 Windows 行为时,优先调用 `KITSUNE_BIN_PATH` 和 CLI 包装命令。原始 socket 客户端要自己负责使用平台原生的本地 socket 形式。
