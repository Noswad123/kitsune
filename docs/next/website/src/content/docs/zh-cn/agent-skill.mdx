...
这份技能告诉智能体: 当 `KITSUNE_ENV=1` 已设置时,使用 `herdr` CLI。这意味着智能体运行在 Herdr 管理的窗格内,可以安全地与本地 Herdr socket 通信。
...
也可以在 Herdr 窗格里使用任何其他编程智能体。重点是智能体进程要运行在 Herdr 内部,这样 `KITSUNE_ENV=1` 才可用。

## 安全规则

这份技能以一条护栏开头: 如果 `KITSUNE_ENV=1` 未设置,智能体应当停下来,并说明自己没有运行在 Herdr 管理的窗格内。
