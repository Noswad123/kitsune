...
Herdr 二进制的 `KITSUNE_BIN_PATH` 调用 Herdr,这样插件在 Unix socket 和
...
在命令内部通过 `KITSUNE_BIN_PATH` 回调 Herdr:
...
`KITSUNE_PLUGIN_STATE_DIR`,再由启动钩子读取并重新应用。

启动钩子会收到普通运行时插件环境和 `KITSUNE_PLUGIN_EVENT=startup`。安装预览会
列出所有启动命令,让用户检查将自动运行的代码。

## 命令与环境

运行时命令以插件目录为工作目录执行。Herdr 注入 `KITSUNE_SOCKET_PATH`、
`KITSUNE_BIN_PATH`、`KITSUNE_ENV=1`、`KITSUNE_PLUGIN_ID`、`KITSUNE_PLUGIN_ROOT`、
`KITSUNE_PLUGIN_CONFIG_DIR`、`KITSUNE_PLUGIN_STATE_DIR`、
`KITSUNE_PLUGIN_CONTEXT_JSON`,以及可用时的 `KITSUNE_WORKSPACE_ID`、
`KITSUNE_TAB_ID` 和 `KITSUNE_PANE_ID`。动作命令还会收到
`KITSUNE_PLUGIN_ACTION_ID`;启动钩子和事件钩子收到 `KITSUNE_PLUGIN_EVENT`
（启动钩子中的值为 `startup`）,事件钩子还会收到
`KITSUNE_PLUGIN_EVENT_JSON`;窗格命令收到 `KITSUNE_PLUGIN_ENTRYPOINT_ID`。

`KITSUNE_PLUGIN_ROOT` 是已安装或已链接的插件目录。不要把用户凭据或持久
状态放在那里,因为 GitHub 安装的插件根目录是托管的源码检出。把 `.env`
这类用户可编辑的配置放在 `KITSUNE_PLUGIN_CONFIG_DIR` 下,把本地运行时
状态放在 `KITSUNE_PLUGIN_STATE_DIR` 下。Herdr 会创建这些目录,并在旧版
插件配置位置存在时把内容初始化到 `KITSUNE_PLUGIN_CONFIG_DIR`,但不会校验、
同步或删除其中的内容。文件格式和生命周期归插件所有。

`KITSUNE_PLUGIN_CONTEXT_JSON` 在本次调用可用时,可以包含工作区、标签页、
...
使用 `KITSUNE_BIN_PATH`。`KITSUNE_SOCKET_PATH` 背后的原始 socket 传输是
操作系统相关的: Unix 客户端连接 Unix socket 路径,Windows 客户端连接
命名管道。通过 `KITSUNE_BIN_PATH` 的 CLI 调用可以避开这种传输差异。可用
...
其进程不会收到 `KITSUNE_PANE_ID`;底层平铺窗格仍可通过 `KITSUNE_PLUGIN_CONTEXT_JSON` 获取。
...
指向同一插件声明的动作。链接处理器动作在 `KITSUNE_PLUGIN_CONTEXT_JSON`
中收到 `invocation_source = "link_click"`、`clicked_url` 和
`link_handler_id`;shell 插件也可以读取 `KITSUNE_PLUGIN_CLICKED_URL` 和
`KITSUNE_PLUGIN_LINK_HANDLER_ID`。每个插件内的处理器按清单顺序检查。
