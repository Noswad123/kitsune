...
弹窗命令不会收到 `KITSUNE_PANE_ID`;可使用 `KITSUNE_ACTIVE_PANE_ID` 引用底层平铺窗格。
...
在相应值可用时，自定义命令会收到 `KITSUNE_SOCKET_PATH`、`KITSUNE_BIN_PATH`、`KITSUNE_ACTIVE_WORKSPACE_ID`、`KITSUNE_ACTIVE_TAB_ID`、`KITSUNE_ACTIVE_PANE_ID` 和 `KITSUNE_ACTIVE_PANE_CWD`。当 Herdr 能检测到聚焦窗格的工作目录时，shell 命令会从该目录运行。

在 Windows 上，自定义命令字符串使用 `cmd.exe /d /c`，因此环境变量应采用 `%KITSUNE_BIN_PATH%` 语法。若要运行 PowerShell 语法，请显式调用它，例如 `powershell.exe -NoProfile -Command "..."`。
...
| `KITSUNE_SOCKET_PATH` | 覆盖底层 socket 路径。 |
| `KITSUNE_LOG` | 设置日志过滤，例如 `KITSUNE_LOG=herdr=debug`。 |
| `KITSUNE_DISABLE_SOUND` | 即使 `[ui.sound] enabled = true` 也禁用声音播放。 |
