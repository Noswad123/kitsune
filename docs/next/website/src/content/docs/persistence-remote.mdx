...
For local builds or custom binaries, set `KITSUNE_REMOTE_BINARY` to a local file path before running remote attach.

```bash
KITSUNE_REMOTE_BINARY=target/release/herdr herdr --remote workbox
```
