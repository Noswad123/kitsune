...
can run too. Most plugins should call Herdr through `KITSUNE_BIN_PATH`, which
...
Inside the command, call back into Herdr with `KITSUNE_BIN_PATH`:

```js
const { spawnSync } = require("node:child_process");

const herdr = process.env.KITSUNE_BIN_PATH ?? "herdr";
...
`KITSUNE_PLUGIN_STATE_DIR`, then read and reapply that view from its startup hook.

Startup hooks receive the normal runtime plugin environment and
`KITSUNE_PLUGIN_EVENT=startup`. The install preview lists every startup command so
...
injects `KITSUNE_SOCKET_PATH`, `KITSUNE_BIN_PATH`, `KITSUNE_ENV=1`,
`KITSUNE_PLUGIN_ID`, `KITSUNE_PLUGIN_ROOT`, `KITSUNE_PLUGIN_CONFIG_DIR`,
`KITSUNE_PLUGIN_STATE_DIR`, `KITSUNE_PLUGIN_CONTEXT_JSON`, and any available
`KITSUNE_WORKSPACE_ID`, `KITSUNE_TAB_ID`, and `KITSUNE_PANE_ID`. Action commands also
receive `KITSUNE_PLUGIN_ACTION_ID`; startup and event hooks receive
`KITSUNE_PLUGIN_EVENT` (`startup` for startup hooks), event hooks additionally
receive `KITSUNE_PLUGIN_EVENT_JSON`, and pane commands receive
`KITSUNE_PLUGIN_ENTRYPOINT_ID`.

`KITSUNE_PLUGIN_ROOT` is the installed or linked plugin directory. Do not store
...
under `KITSUNE_PLUGIN_CONFIG_DIR`, and put local runtime state under
`KITSUNE_PLUGIN_STATE_DIR`. Herdr creates those directories and seeds
`KITSUNE_PLUGIN_CONFIG_DIR` from the legacy plugin config locations when present,
but it does not validate, sync, or delete their contents. The plugin owns the
file format and lifecycle.

`KITSUNE_PLUGIN_CONTEXT_JSON` can include workspace, tab, focused pane, worktree,
...
Use `KITSUNE_BIN_PATH` when a plugin needs to call Herdr portably from Node,
PowerShell, Bash, or another runtime. The raw socket transport behind
`KITSUNE_SOCKET_PATH` is OS-specific: Unix clients connect to a Unix socket path,
while Windows clients connect to a named pipe. CLI calls through
`KITSUNE_BIN_PATH` avoid that transport difference. See the
...
Its process does not receive `KITSUNE_PANE_ID`; the underlying tiled pane remains
available through `KITSUNE_PLUGIN_CONTEXT_JSON`.
...
`KITSUNE_PLUGIN_CONTEXT_JSON`; shell plugins can also read
`KITSUNE_PLUGIN_CLICKED_URL` and `KITSUNE_PLUGIN_LINK_HANDLER_ID`. Handlers are
