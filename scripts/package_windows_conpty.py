...
MARKER_PATH = PurePosixPath("conpty/kitsune-conpty.json")
...
def stage_bundle(
    ...
    if not kitsune_exe.is_file():
        raise ValueError(f"Kitsune executable does not exist: {kitsune_exe}")
    ...
    with zipfile.ZipFile(package_path) as archive, tempfile.TemporaryDirectory(
        prefix="kitsune-conpty-stage-", dir=output_dir.parent
    ) as temporary:
        validate_nuspec(archive, metadata["package"])
        staging = Path(temporary) / "bundle"
        staging.mkdir()
        shutil.copy2(kitsune_exe, staging / "kitsune.exe")
...
def expected_stage_files(metadata: dict[str, Any], architecture: str) -> set[str]:
    files = {"kitsune.exe", MARKER_PATH.as_posix()}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Package Kitsune with Microsoft's pinned ConPTY runtime")
    ...
    stage.add_argument("--kitsune-exe", type=Path, required=True)
