...
///
/// Contractual override behavior for auto-detect uses `KITSUNE_SOCKET_PATH`.
/// This variable is kept as a fallback for callers that explicitly need a
/// client-only override when `KITSUNE_SOCKET_PATH` is not set.
pub const CLIENT_SOCKET_PATH_ENV_VAR: &str = crate::product::CLIENT_SOCKET_PATH_ENV_VAR;
...
/// 2. If `KITSUNE_SOCKET_PATH` is set, derive the client socket path from it by
///    inserting `-client` before `.sock` (e.g. `kitsune.sock` -> `kitsune-client.sock`).
///    This keeps JSON API and client socket overrides consistent.
/// 3. Otherwise, honor `KITSUNE_CLIENT_SOCKET_PATH` (legacy/testing fallback).
...
pub(crate) fn derive_client_socket_from_api_socket(api_socket_path: &Path) -> PathBuf {
    let stem = api_socket_path
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or(crate::product::CONFIG_DIR_NAME);
...
pub(crate) fn prepare_socket_path(path: &Path) -> io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "kitsune server is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn client_socket_path_derived_from_api_socket_override() {
        let path = client_socket_path_from_overrides(Some("/tmp/test-kitsune.sock"), None);
        assert_eq!(path, PathBuf::from("/tmp/test-kitsune-client.sock"));
    }
    ...
    fn client_socket_path_respects_legacy_client_override_without_api_override() {
        let path = client_socket_path_from_overrides(None, Some("/tmp/test-kitsune-client.sock"));
        assert_eq!(path, PathBuf::from("/tmp/test-kitsune-client.sock"));
    }
    ...
    fn client_socket_path_defaults_to_config_dir() {
        ...
        assert_eq!(
            path,
            crate::config::config_dir().join(crate::product::CLIENT_SOCKET_FILE_NAME)
        );
    }
