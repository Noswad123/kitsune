...
pub(crate) fn handoff_socket_path() -> PathBuf {
    crate::session::data_dir().join(format!("kitsune-handoff-{}.sock", std::process::id()))
}
...
pub(crate) fn spawn_handoff_import(
    ...
    let exe = if let Some(import_exe) = import_exe {
        ...
        fallback_exe = std::env::current_exe().map_err(|err| {
            io::Error::new(
                err.kind(),
                format!("failed to determine kitsune executable path: {err}"),
            )
        })?;
        &fallback_exe
    };
    ...
    if crate::session::explicit_session_requested() {
        ...
        // stale socket overrides must not mask the inherited KITSUNE_SESSION.
        command
...
pub(crate) fn receive(socket_path: &Path, token: &str) -> io::Result<ReceivedHandoff> {
    ...
    {
        return Err(io::Error::other(format!(
            "handoff expected kitsune v{}, but this server is v{}",
