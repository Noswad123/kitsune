...
fn generated_protocol_schema_artifact_is_current() {
    ...
    let expected = std::fs::read_to_string(&path).unwrap_or_else(|err| {
        panic!(
            "failed to read {}; run `KITSUNE_UPDATE_API_SCHEMA=1 just test-one generated_protocol_schema_artifact_is_current`: {err}",
            path.display()
        )
    });
    assert_eq!(
        expected,
        actual,
        "generated API schema artifact is stale; run `KITSUNE_UPDATE_API_SCHEMA=1 just test-one generated_protocol_schema_artifact_is_current`"
    );
}
    ...
    let root = LayoutNode::Split {
        ...
        second: Box::new(LayoutNode::Pane {
            pane: LayoutPane {
                ...
                env: HashMap::from([("KITSUNE_ROLE".into(), "tests".into())]),
                ..Default::default()
            },
        }),
    };
