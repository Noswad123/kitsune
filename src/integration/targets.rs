...
pub(crate) fn remove_legacy_pi_extension_from_omp_dir(dir: &Path) -> io::Result<bool> {
    ...
    if content.contains("KITSUNE_INTEGRATION_ID=pi") {
...
/// The complete Kitsune-owned Grok hook config. Installation and status share
...
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    ...
    // Grok merges every `~/.grok/hooks/*.json`, so kitsune owns a dedicated
...
pub(crate) fn uninstall_grok() -> io::Result<GrokUninstallResult> {
    ...
    // kitsune owns both files outright, so removal is a straight delete.
