...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!(
                    "{}",
                    crate::product::usage("agent explain <target> [--json|--verbose]")
                );
                eprintln!(
                    "{}",
                    crate::product::usage(
                        "agent explain --file PATH --agent LABEL [--json|--verbose]"
                    )
                );
                return Ok(0);
            }
            ...
            value => {
                if target.is_some() {
                    eprintln!(
                        "{}",
                        crate::product::usage("agent explain <target> [--json]")
                    );
                    return Ok(2);
                }
    ...
    let explain = if let Some(path) = file {
        if target.is_some() {
            eprintln!(
                "{}",
                crate::product::usage("agent explain --file PATH --agent LABEL [--json]")
            );
            return Ok(2);
        }
        let Some(agent_label) = agent else {
            eprintln!(
                "{} requires --agent LABEL",
                crate::product::command("agent explain --file")
            );
            return Ok(2);
        };
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("{}", crate::product::usage("agent list"));
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("{}", crate::product::usage("agent get <target>"));
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("{}", crate::product::usage("agent get <target>"));
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("{}", crate::product::usage("agent focus <target>"));
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("{}", crate::product::usage("agent focus <target>"));
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) = match super::parse_attach_target(
        args,
        &crate::product::usage("agent attach <target> [--takeover]"),
    ) {
        Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!(
            "{}",
            crate::product::usage("agent wait <target> [--until STATUS]... [--timeout MS]")
        );
        return Ok(2);
    };
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let [target, value] = args else {
        eprintln!(
            "{}",
            crate::product::usage("agent rename <target> <name>|--clear")
        );
        return Ok(2);
    };
...
fn agent_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!(
            "{}",
            crate::product::usage("agent send-keys <target> <key> [key ...]")
        );
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("{}", crate::product::usage("agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]"));
        return Ok(2);
    };
...
fn print_agent_help() {
    ...
    eprintln!(
        "  {}",
        crate::product::command("agent read <target> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi]")
    );
    eprintln!(
        "  {}",
        crate::product::command("agent send-keys <target> <key> [key ...]")
    );
    eprintln!(
        "  {}",
    ...
    eprintln!(
        "  {}",
        crate::product::command("agent rename <target> <name>|--clear")
    );
    eprintln!("  {}", crate::product::command("agent focus <target>"));
    eprintln!(
        "  {}",
        crate::product::command("agent wait <target> [--until STATUS]... [--timeout MS]")
    );
    eprintln!(
        "  {}",
        crate::product::command("agent attach <target> [--takeover]")
    );
    eprintln!(
        "  {}",
        crate::product::command(
    ...
    eprintln!(
        "  {}",
        crate::product::command("agent explain <target> [--json|--format text|json] [--verbose]")
    );
    eprintln!(
        "  {}",
        crate::product::command(
            "agent explain --file PATH --agent LABEL [--json|--format text|json] [--verbose]"
        )
    );
